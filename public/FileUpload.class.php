...
class FileUpload {
	...
	//显示图片加赋值(上传专用)
	public function showImg($_path) {
		echo "<script>showimg=parent.document.getElementById('showimg');showimg.src='.{$_path}'</script>";
		echo "<script>thumbnail=parent.document.getElementById('thumbnail');thumbnail.value='.{$_path}'</script>";
	}
}
?>